...
def backpropagation_training(l, ETA, desired_epochs, learning_mode):
    ...
    if W0 is None or W1 is None:
        W0 = np.random.uniform(low=-1, high=1, size=(2, INPUT_DIMENSION))
        W1 = np.random.uniform(low=-1, high=1, size=(OUTPUT_DIMENSION, 2))

    if B0 is None or B1 is None:
        B0 = np.random.uniform(low=-1, high=1, size=(2, 1))
        B1 = np.random.uniform(low=-1, high=1, size=(OUTPUT_DIMENSION, 1))
    ...
    while epochs < desired_epochs:
        ...
        for e in range(0, min(desired_epochs, SUB_STEP)):
            WB0, WB1, E_epoch = gradient_descent_algorithm(D, WB0, WB1, ETA, epochs + e, learning_mode)
            E[e] = E_epoch
